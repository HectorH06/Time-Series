...
forecast_ets <- fit_ets |> forecast(h = "2 years")
autoplot(tasmania, Trips) + 
  autolayer(forecast_ets)
...
```{r}
fit_arima <- tasmania |> 
  model(ARIMA(Trips ~ pdq(1,1,1) + PDQ(0,1,0)))

forecast_arima <- fit_arima |> forecast(h = "2 years")
autoplot(tasmania, Trips) + 
  autolayer(forecast_arima)
```
...
```
```{r}
fit_ets_gas <- gas_train |> 
  model(ETS(Gas))

fit_arima_gas <- gas_train |> 
  model(ARIMA(Gas))
```
```{r}
fit_drift_gas <- gas_train |> 
  model(RW(Gas ~ drift()))

accuracy(fit_drift_gas)
accuracy(fit_ets_gas)
accuracy(fit_arima_gas)
```

```{r}
forecast_ets_gas <- fit_ets_gas |> forecast(h = "2 years")
forecast_arima_gas <- fit_arima_gas |> forecast(h = "2 years")
forecast_drift_gas <- fit_drift_gas |> forecast(h = "2 years")

autoplot(gas_train, Gas) +
  autolayer(forecast_ets_gas, series = "ETS", PI = TRUE) +
  ggtitle("ETS") +
  ylab("Consumo de Gas") +
  xlab("Q1")

autoplot(gas_train, Gas) +
  autolayer(forecast_arima_gas, series = "ARIMA", PI = TRUE) +
  ggtitle("ARIMA") +
  ylab("Consumo de Gas") +
  xlab("Q1")
  
autoplot(gas_train, Gas) +
  autolayer(forecast_drift_gas, series = "Drift", PI = TRUE) +
  ggtitle("Drift zZzzZ") +
  ylab("Consumo de Gas") +
  xlab("Q1")
